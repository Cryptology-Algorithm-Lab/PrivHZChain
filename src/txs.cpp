...
Tx_Entry Tx_Entry_Gen (
    ...
    G2 C = pcs.commit_g2({id, 1}) + pcs.pp.h2si[0] * gamma;
    G2 A = pcs.commit_g2(constructMemPoly(pcs.pp.nttctx, SNs)) + pcs.pp.h2si[0] * delta;
...
Tx_Exit Tx_Exit_Gen (
    ...
    // Line 1
    G1 w1 = pcs.commit_g1(constructMemPolyBatch(pcs.pp.nttctx, S, S_1));
    G1 w2 = pcs.commit_g1(constructMemPolyBatch(pcs.pp.nttctx, S, S_2));

    // TxCom
    auto time1 = chrono::steady_clock::now();

    G2 Cp = pcs.commit_g2({id, 1}) + pcs.pp.h2si[0] * gammap;
    G2 A1 = pcs.commit_g2(constructMemPoly(pcs.pp.nttctx, S_1)) + pcs.pp.h2si[0] * delta1;
    G2 A2 = pcs.commit_g2(constructMemPoly(pcs.pp.nttctx, S_2)) + pcs.pp.h2si[0] * delta2;
    ...
    PoID.prove(Tx_in.C - pcs.pp.g2si[1], id, gamma);
...
bool Tx_Exit_Vrfy (
    ...
    // Check PoID
    bool checkPoID = PoK2_verify(&tx.PoID);
...
Tx_Trans Tx_Trans_Gen (
    ...
    G2 C1 = pcs.commit_g2({id1, 1}) + pcs.pp.h2si[0] * gamma1;
    G2 C2 = pcs.commit_g2({id2, 1}) + pcs.pp.h2si[0] * gamma2;
    G2 A1 = pcs.commit_g2(constructMemPoly(pcs.pp.nttctx, S_1)) + pcs.pp.h2si[0] * delta1;
    G2 A2 = pcs.commit_g2(constructMemPoly(pcs.pp.nttctx, S_2)) + pcs.pp.h2si[0] * delta2;
    ...
    PoID.prove(Tx_in.C - pcs.pp.g2si[1], id1, gamma);
...
Tx_Summary Tx_Summary_Gen (
    ...
    FrVec SN_memPoly = constructMemPoly(pcs.pp.nttctx, SN_loc);
    G2 C_locp = pcs.commit_g2(SN_memPoly) + pcs.pp.h2si[0] * gamma_locp;
